...
def Read_Txt(command='1'):
    ...
    def exls_w_adr(row=1, column=1, list_str=None, bold=False):
        ...
        for cel in list_str:
            if cel.find('Адрес:') != -1: column = 1
            if cel.find('Тип_прибора: ') != -1: cel = str(device_type[int(cel[cel.find('Тип_прибора: '):].replace('Тип_прибора: ', ''))]).strip('[]').strip('\'\'')
            ...
            if cel.find('Сценарий_упр:') != -1: column = 3
            if cel.find('Версия:') != -1: column = 4
    ...
    def exls_w_out(row=1, column=1, list_str=None, bold=False):
        ...
        for cel in list_str:
            ...
            if cel.find('Шлейф:') != -1:
                ...
                if 20 < cel.__len__() < 30: ws.row_dimensions[row].height = 25
                if cel.__len__() > 30: ws.row_dimensions[row].height = 37
                column = 3
            ...
            '''
                ...
                cel = 'Пр. упр: ' + str(type_sc[int(int_sc)]).strip('[]').strip('\'\'')
                print(int_sc)
                ws.row_dimensions[row].height = 37
                column = 3
'''
    ...
    def exls_w_titul(row=1, column=1, list_str=None, bold=False):
        ...
        for cel in list_str:
            ...
            cells.font = Font(name='Times New Roman', size=10, bold=bold, italic=True)
            #   TODO    если четная строка
            cells.fill = PatternFill(fill_type='solid', fgColor='F0F8FF') if row % 2 == 0 else PatternFill(fill_type='solid', fgColor='FFFAF0')
            column += 1
        row += 1
        return row
    ...
    def Exls_w_sc(row=1, column=1, list_str=None, bold=False, group_start=1):
        ...
        for cel in list_str:
            if cel.find('Сценарий_упр:') != -1:
                ...
                column = 3
            elif cel.find('Исх. маска мигания:') != -1: column = 4
            elif cel.find('Шаг:') != -1:
                ws.row_dimensions[row].height = 37
                column = 2
            elif cel.find('Программа:') != -1 and str(ws.cell(row, 2).value).find('Шаг:') != -1:
                int_sc = int(cel.replace(' ', '').replace('Программа:', ''))
                if temp == 'управление реле': cel = 'Пр. упр: ' + str(relay_script[int(int_sc)]).strip('[]').strip('\'\'')
                if temp == 'управление шлейфом': cel = 'Пр. упр: ' + str(cable_script[int(int_sc)]).strip('[]').strip('\'\'')
                ws.row_dimensions[row].height = 37
                column = 3
